...
<ref:refname>db-label.label</ref:refname>
...
<ref:para>
  ...
  or Table 5-2.  The template simply applies the mode
  <ref:function>db-label.label.mode</ref:function> to the node.  To change
  the behavior of a particular type of element, then, you should always
  override the mode template for that type of element.
</ref:para>
<ref:para>
  Overriding the <ref:function>db-label.label</ref:function> template should
  only be done if you wish to change the labelling mechanism completely, or
  you wish to wrap the labelling mechanism (for instance, with a cacheing
  extension).  Do not override this template to suppress label prefixes in
  titles.
</ref:para>
<ref:para>
  Most labels contain a name portion and a number portion.  Templates should
  generally call <ref:function>db-label.name</ref:function> and
  <ref:function>db-label.number</ref:function> to generate these portions.
</ref:para>

<xsl:template name="db-label.label">
  ...
  <xsl:choose>
    ...
    <xsl:otherwise>
      <xsl:apply-templates mode="db-label.label.mode" select="$node"/>
    </xsl:otherwise>
  </xsl:choose>
</xsl:template>

<xsl:template mode="db-label.label.mode" match="
...
<xsl:template mode="db-label.label.mode" match="synopfragment">
  <xsl:text>(</xsl:text>
  <xsl:call-template name="db-label.number"/>
  <xsl:text>)</xsl:text>
</xsl:template>

<xsl:template mode="db-label.label.mode" match="title | subtitle">
  <xsl:call-template name="db-label.label">
...
<ref:refname>db-label.name</ref:refname>
...
<ref:para>
  ...
  sectioning and block-level elements.  The template simply applies the mode
  <ref:function>db-label.name.mode</ref:function> to the node.  To change
  the behavior of a particular type of element, then, you should always
  override the mode template for that type of element.
</ref:para>
<ref:para>
  Overriding the <ref:function>db-label.name</ref:function> template should
  only be done if you wish to change the naming mechanism completely, or
  you wish to wrap the naming mechanism (for instance, with a cacheing
  extension).
</ref:para>
...
<ref:refname>db-label.number</ref:refname>
...
<ref:para>
  ...
  sectioning and block-level elements.  The template simply applies the mode
  <ref:function>db-label.number.mode</ref:function> to the node.  To change
  the behavior of a particular type of element, then, you should always
  override the mode template for that type of element.
</ref:para>
<ref:para>
  Overriding the <ref:function>db-label.number</ref:function> template should
  only be done if you wish to change the numbering mechanism completely, or
  you wish to wrap the numbering mechanism (for instance, with a cacheing
  extension).
</ref:para>
